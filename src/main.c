...
)
{
    ...
    int x = NormalizeTileX(requested_x, zoom);

    MapTile *cached = FindCachedTile(zoom, x, y);
    if (cached != NULL) {
        return cached;
    }
    ...
    if (!FileExists(path)) {
        ...
        if (!DownloadFile(url, path)) {
            fprintf(stderr, "Failed to download tile %d/%d/%d\n", zoom, x, y);
            return NULL;
        }
    }
...
static void EnsureVisibleTiles(Camera2D camera, int zoom, const char *api_key)
{
    int min_x = 0;
    int max_x = 0;
    int min_y = 0;
...
int main(void)
{
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_ONE)) chosen_type = 1;
        if (IsKeyPressed(KEY_TWO)) chosen_type = 2;
        if (IsKeyPressed(KEY_THREE)) chosen_type = 3;
        ...
        EnsureVisibleTiles(camera, map_zoom, api_key);
